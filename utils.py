...
def render_repository_metrics(repository_metrics):
    ...
    for header in headers["values"]:
        ...
        for file_path, metrics in repository_metrics.items():
            ...
            if header == "File name":
                ...
                output_cols[f"{file_path}"].append(" ".join(map(str, metrics[header])))
            else:
                col.append(metrics[header])
                total_values[header] += metrics[header]
                output_cols[f"{file_path}"].append(metrics[header])

        if header == "Highest CC function":
            col.append(" ".join(map(str, total_values[header])))
            output_cols[f"{file_path}"].append(" ".join(map(str, total_values[header])))
            ...
            output_cols[f"{file_path}"].append("Avg F: %.2f" % avg_comment_level)
        else:
            col.append(total_values[header])
            output_cols[f"{file_path}"].append(total_values[header])
    ...
    print("File name | LOC | Empty LOC | Physical LOC | Logical LOC | Comment Lines | Comment level (F)")   
    
    for entry in output_cols:
        test = f"{entry}: " 
        for cell in enumerate(output_cols[entry]):
            test += f"{cell}" + ', '
        test += '\n'
